...
class EditorController < ApplicationController
  ...
  def page
    ...
    page_param[:menu].each_with_index do |menu, index|
      ...
      parent.update(position: index, parent: nil, display: menu['display'])
      next if menu[:nested].empty?

      menu[:nested].each_with_index do |sub_menu, index|
        page = Page.find sub_menu[:id]
        page.update!(position: index, parent: parent, display: sub_menu['display'])
      end
    end
  end
  ...
  def page_param
    params.permit(menu: [:id, :title, :display, :position, :seo, :created_at, :updated_at, :fb_page_template_id, :uri, nested: %i[id title display position seo created_at updated_at fb_page_template_id uri]])
  end
